...
function compareVersionNumbers(v1, v2) {
    // set up variables
    let testv1 = v1.split('.'), testv2 = v2.split('.'), i = 0
    // iterate through version numbers
    while (i < testv1.length && i < testv2.length){
        let v1num = parseInt(testv1[i]), v2num = parseInt(testv2[i])
    ...
    // compare version numbers lengths
    if ( testv1.length > testv2.length ) {
        return 1
    } else if ( testv1.length < testv2.length ) {
        return -1
    } else {
        return 0
    }
